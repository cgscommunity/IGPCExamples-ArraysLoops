...
int main()
{
	...
	do
	{
		...
		std::cout << "nter an integer between 10 and 20: ";
		std::cin >> numEntered;
		...
		else
		{
			...
			std::cout << "That number is not between 10 and 20, please try again." << endl;
		}

	} while (!isValid); // Continue loop as long as isValid == false

	// Final message printed now the loop is over
	std::cout << "Thanks for entering " << numEntered << ", that number is between 10 and 20." << endl;
}